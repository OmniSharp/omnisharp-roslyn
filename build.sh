...
dnu publish src/OmniSharp --configuration Release --no-source --out artifacts/build/omnisharp --runtime dnx-mono.1.0.0-beta5 2>&1 | tee buildlog
...
mono nuget.exe install dnx-clr-win-x86 -Version 1.0.0-beta5 -Prerelease -OutputDirectory artifacts/build/omnisharp/approot/packages

if [ ! -d "artifacts/build/omnisharp/approot/packages/dnx-clr-win-x86.1.0.0-beta5" ]; then
    echo 'ERROR: Can not find dnx-clr-win-x86.1.0.0-beta5 in output exiting!'
    exit 1
fi

if [ ! -d "artifacts/build/omnisharp/approot/packages/dnx-mono.1.0.0-beta5" ]; then
    echo 'ERROR: Can not find dnx-mono.1.0.0-beta5 in output exiting!'
    exit 1
fi
