...
dnu publish src/OmniSharp --configuration Release --no-source --out artifacts/build/omnisharp --runtime dnx-mono.1.0.0-beta8 2>&1 | tee buildlog
...
if [ ! -d "artifacts/build/omnisharp/approot/packages/dnx-mono.1.0.0-beta8" ]; then
    echo 'ERROR: Can not find dnx-mono.1.0.0-beta8 in output exiting!'
    exit 1
fi
