...
public class HighlightFacts
    : AbstractSingleRequestHandlerTestFixture<HighlightingService> {
...
");
    ...
    AssertSyntax(highlights, testFile.Content.Code, 3, Keyword("class"),
                 ClassName("C1"), Punctuation("{"), Keyword("int"), Field("n"),
                 Operator("="), Keyword("true"), Punctuation(";"),
                 Punctuation("}"));
  }
...
");
    ...
    AssertSyntax(highlights, testFile.Content.Code, 0, Keyword("namespace"),
                 NamespaceName("N1"), Punctuation("{"), Keyword("class"),
                 ClassName("C1"), Punctuation("{"), Keyword("int"), Field("n"),
                 Operator("="), Keyword("true"), Punctuation(";"),
...
");
    ...
    AssertSyntax(highlights, testFile.Content.Code, 0, Keyword("class"),
                 ClassName("C1"), Punctuation("{"), Keyword("string"),
                 Field("s"), Operator("="), String("$\""), Punctuation("{"),
                 Number("5"), Punctuation("}"), String("\""), Punctuation(";"),
                 Punctuation("}"));
  }
...
");
    ...
    var highlights = await GetHighlightsAsync(
        testFile, exclude: HighlightClassification.Keyword);
...
");
    ...
    var highlights = await GetHighlightsAsync(
        testFile, exclude: HighlightClassification.Punctuation);
...
");
    ...
    var highlights = await GetHighlightsAsync(
        testFile, exclude: HighlightClassification.Operator);
...
");
    ...
    var highlights = await GetHighlightsAsync(
        testFile, exclude: HighlightClassification.Identifier);
...
");
    ...
    var highlights = await GetHighlightsAsync(
        testFile, exclude: HighlightClassification.Name);
...
");
    ...
    foreach (var additiveName in Microsoft.CodeAnalysis.Classification
                 .ClassificationTypeNames.AdditiveTypeNames) {
      Assert.DoesNotContain(highlights, x => x.Kind == additiveName);
    }
  }

  private async Task<HighlightSpan[]>
  GetHighlightsAsync(TestFile testFile, int? line = null,
  ...
  private static void
  AssertSyntax(HighlightSpan[] highlights, string code, int startLine,
  ...
  private static (string kind, string text)
      ClassName(string text) => ("class_name", text);
  private static (string kind, string text)
      Field(string text) => ("field_name", text);
  private static (string kind, string text)
      Identifier(string text) => ("identifier", text);
  private static (string kind, string text)
      NamespaceName(string text) => ("namespace_name", text);
  private static (string kind, string text)
      Keyword(string text) => ("keyword", text);
  private static (string kind, string text)
      Number(string text) => ("number", text);
  private static (string kind, string text)
      Operator(string text) => ("operator", text);
  private static (string kind, string text)
      Punctuation(string text) => ("punctuation", text);
  private static (string kind, string text)
