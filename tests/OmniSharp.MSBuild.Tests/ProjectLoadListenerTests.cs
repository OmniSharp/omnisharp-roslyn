...
namespace OmniSharp.MSBuild.Tests
{
    public partial class ProjectLoadListenerTests : AbstractMSBuildTestFixture
    {
        ...
        public async Task The_correct_sdk_version_is_emitted_NET8()
        {
            ...
            using var host = CreateMSBuildTestHost(testProject.Directory, emitter.AsExportDescriptionProvider(LoggerFactory));
            Assert.Single(emitter.ReceivedMessages);
            Assert.Equal(GetHashedFileExtension("8.0.408"), emitter.ReceivedMessages[0].SdkVersion);
        }
        ...
        public async Task The_correct_sdk_version_is_emitted_NET9()
        {
            ...
            using var host = CreateMSBuildTestHost(testProject.Directory, emitter.AsExportDescriptionProvider(LoggerFactory));
            Assert.Single(emitter.ReceivedMessages);
            Assert.Equal(GetHashedFileExtension("9.0.203"), emitter.ReceivedMessages[0].SdkVersion);
        }
        ...
        public async Task The_correct_sdk_version_is_emitted_NET10()
        {
            ...
            using var host = CreateMSBuildTestHost(testProject.Directory, emitter.AsExportDescriptionProvider(LoggerFactory));
            Assert.Single(emitter.ReceivedMessages);
            Assert.Equal(GetHashedFileExtension("10.0.100-preview.3.25201.16"), emitter.ReceivedMessages[0].SdkVersion);
        }
