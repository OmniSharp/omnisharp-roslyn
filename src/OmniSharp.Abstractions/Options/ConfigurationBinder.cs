...
//TODO: Replace with library once we get to the appropriate beta.
...
namespace Microsoft.Framework.Configuration
{
    public static class ConfigurationBinder
    {
        ...
        private static void BindProperty(PropertyInfo property, object instance, IConfiguration config)
        {
            ...
            if (!config.GetSubKeys().Any())
            {
                var configValue = config.Get(property.Name);
                if (string.IsNullOrEmpty(configValue))
                {
                    // Leaf nodes are always reinitialized
                    ReadValue(property.PropertyType, config.Get(property.Name), config);
                    return;
                }
            }
            else
            {
                propertyValue = BindInstance(property.PropertyType, propertyValue, config.GetSubKey(property.Name));
                if (propertyValue != null && hasPublicSetter)
                {
        ...
        private static void BindDictionary(object dictionary, Type dictionaryType, IConfiguration config)
        {
            ...
            foreach (var child in config.GetSubKeys())
            {
                ...
                if (item != null)
                {
                    ...
                    key = key.Substring(child.Key.Length + 1);
        ...
        private static void BindCollection(object collection, Type collectionType, IConfiguration config)
        {
            ...
            foreach (var section in config.GetSubKeys())
            {
                try
                {
                    var item = BindInstance(
        ...
        {
            var children = config.GetSubKeys().ToArray();
