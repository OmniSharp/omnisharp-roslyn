    ...
    public abstract class BaseCodeActionService<TRequest, TResponse> : IRequestHandler<TRequest, TResponse>
    {
        ...
        protected async Task<(Solution Solution, IEnumerable<FileOperationResponse> FileChanges)> GetFileChangesAsync(Solution newSolution, Solution oldSolution, string directory, bool wantTextChanges, bool wantsAllCodeActionOperations)
        {
            ...
            foreach (var projectChange in solutionChanges.GetProjectChanges())
            {
                ...
                var changedDocuments = new List<(DocumentType Type, DocumentId Id)>();
                changedDocuments.AddRange(projectChange.GetChangedDocuments().Select(docId => (DocumentType.Regular, docId)));
                changedDocuments.AddRange(projectChange.GetChangedAdditionalDocuments().Select(docId => (DocumentType.Additional, docId)));

                // Handle changed documents
                foreach (var document in changedDocuments)
                {
                    var newDocument = document.Type == DocumentType.Regular ? newSolution.GetDocument(document.Id) : newSolution.GetAdditionalDocument(document.Id);
                    var oldDocument = document.Type == DocumentType.Regular ? oldSolution.GetDocument(document.Id) : oldSolution.GetAdditionalDocument(document.Id);
                    ...
                    if (oldDocument != null && newDocument.Name != oldDocument.Name)
                    {
                        if (wantsAllCodeActionOperations)
                        {
                            ...
                            var temp = document.Type == DocumentType.Regular ? solution.RemoveDocument(document.Id) : solution.RemoveAdditionalDocument(document.Id);
                            solution = document.Type == DocumentType.Regular
