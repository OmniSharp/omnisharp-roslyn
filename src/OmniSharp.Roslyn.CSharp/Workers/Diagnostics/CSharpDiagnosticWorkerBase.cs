...
public abstract class CSharpDiagnosticWorkerBase : ICsDiagnosticWorker
{
    ...
    private ImmutableArray<Document> FilterDocuments(IEnumerable<Document> documents) =>
        documents
            ...
            .Select(x =>
            {
                using var sw = new StreamWriter("logs.txt");
                var path = FileSystemHelper.GetRelativePath(x.FilePath, _fileSystemHelper.TargetDirectory);
                var match = _fileSystemMatcher.Match(path).HasMatches;
                sw.WriteLine($"{path}: {match}");
                return x;
            })
