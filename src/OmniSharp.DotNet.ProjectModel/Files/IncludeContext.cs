...
namespace Microsoft.DotNet.ProjectModel.Files
{
    public class IncludeContext
    {
            ...
            string[] defaultBuiltInExclude)
        {
            ...
            SourceBasePath = sourceBasePath ?? throw new ArgumentNullException(nameof(sourceBasePath));
            ...
            else
            {
                ...
                if (token.Value<JToken>("builtIns") is JObject builtIns)
                {
                ...
                if (token.Value<JToken>("mappings") is JObject mappings)
                {
